...
struct stralgo
{
    ...
    public:
        ...
        bool endswith(const base_type& suffix) const {
            ...
            for (unsigned int i = base_type::size() - suffix.size(), j = 0;
                    j < suffix.size(); ++i, ++j)
                if (base_type::operator[](i) != suffix[j])
                    return false;
            return true;
        }
    public:
        self_type lower(void) const {
            return stralgo::lower(*this);
        }
        self_type upper(void) const {
            return stralgo::upper(*this);
        }
    public:
    ...
    public:
        ...
        std::vector<self_type>
        split(const base_type& token, int max_split,
              bool compress_token = true) const {
    ...
    public:
        ...
        self_type sprintf(const Args&... args) const {
            return stralgo::sprintf(*this, args...);
        }
        template <typename... Args>
        self_type format(const Args&... args) const {
            return stralgo::format(*this, args...);
        }
    };
...
#define __wrap__(isxxx, ret, arg_t) \
    ...
    __wrap__(isalnum,   bool,    char);
    __wrap__(isalpha,   bool,    char);
    __wrap__(iscntrl,   bool,    char);
    __wrap__(isdigit,   bool,    char);
    __wrap__(isgraph,   bool,    char);
    __wrap__(islower,   bool,    char);
    __wrap__(isprint,   bool,    char);
    __wrap__(ispunct,   bool,    char);
    __wrap__(isspace,   bool,    char);
    __wrap__(isupper,   bool,    char);
    __wrap__(iswalnum,  bool,    wchar_t);
    __wrap__(iswalpha,  bool,    wchar_t);
    __wrap__(iswcntrl,  bool,    wchar_t);
    __wrap__(iswdigit,  bool,    wchar_t);
    __wrap__(iswgraph,  bool,    wchar_t);
    __wrap__(iswlower,  bool,    wchar_t);
    __wrap__(iswprint,  bool,    wchar_t);
    __wrap__(iswpunct,  bool,    wchar_t);
    __wrap__(iswspace,  bool,    wchar_t);
    __wrap__(iswupper,  bool,    wchar_t);
    __wrap__(iswxdigit, bool,    wchar_t);
    __wrap__(isxdigit,  bool,    char);
    __wrap__(tolower,   char,    char);
    __wrap__(toupper,   char,    char);
    __wrap__(towlower,  wchar_t, wchar_t);
    __wrap__(towupper,  wchar_t, wchar_t);
#undef __wrap__

    template <int base = 10, typename CharT = char, typename IntegralType>
    static basic_string<CharT> strf(IntegralType integer)
    {
        ...
        do { result += zero[integer % base]; } while ((integer /= base));
    ...
    static std::string strf(char ch) { return std::string(1, ch); }
    static std::string strf(float number);
    static std::string strf(double number);
    static std::string strf(long double number);
    static std::string strf(bool bl) { return bl ? "true" : "false"; };
    ...
    static const basic_string<char>&
      strf(const basic_string<char>& str) { return str; }
    static basic_string<char>&&
      strf(basic_string<char>&& str) { return std::move(str); }
    ...
    static string format(const std::string& pattern, const Args&... args)
    {
        strings strs;
        strsf(strs, args...);
        return format_ss(pattern.c_str(), strs);
    }

    template <typename IntegralType, int base = 10, bool skip_space = true,
              typename CharT = char>
    static IntegralType frts(const CharT* str)
    {
        static_assert(2 <= base && base <= 36,
                      "`int base' must be between 2 and 36.");
        static_assert(std::is_integral<IntegralType>::value,
    ...
    template <typename IntegralType,
              int base = 10, bool skip_space = true,
              typename CharT>
    static IntegralType frts(const std::basic_string<CharT>& str)
    {
...
#undef atox
    ...
    static wstring decode (const std:: string& text,
                           const std:: string& codec);
    static  string encode (const std::wstring& text,
                           const std:: string& codec);
    static  string convert(const std:: string& text,
                           const std:: string& text_codec,
                           const std:: string& conv_codec);
            ...
            const std::basic_string<CharT>& goal, int max_replace = -1)
    {
        ...
        typename string::size_type pos = 0, lpos = pos;
        ...
        {
            result.append(str.begin() + lpos, str.begin() + pos - lpos);
            result += goal;
            lpos = pos += aim.size();
            ++replaced;
        }

        result.append(str.begin() + lpos, str.end());
          ...
          int max_split, bool compress_token = true)
    {
        ...
        {
            if (!compress_token || pos != lpos || pos == 0)
                result.emplace_back(str.substr(lpos, pos - lpos));

            lpos = pos += token.size();
        }

        result.emplace_back(str.substr(lpos));
        return std::move(result);
    }
    ...
    static std::vector<basic_string<CharT>>
    split(const std::basic_string<CharT>& str,
          const std::basic_string<CharT>& token, bool compress_token = true)
    {
...
private:
    ...
    static bool isfrts(char ch)
    {
        if (base <= 10)
            ...
            return between<'0', base>(ch)
                || between<'a', base - 10>(ch)
                || between<'A', base - 10>(ch);
    }

    template <typename Arg>
    static void strsf(strings& strs, const Arg& arg)
    {
        strs.emplace_back(strf(arg));
    }

    template <typename Arg, typename... Others>
    static void strsf(strings& strs, const Arg& arg, const Others&... others)
    {
        strs.emplace_back(strf(arg));
        strsf(strs, others...);
    }

    static string format_ss(const char* pattern, const strings& strs);
};

stralgo::string operator""_ts(const char*, size_t);
