...
public:
    ...
    pixel_format pixel(int shifted_x, int shifted_y) const
    {
        ...
        const data_type& lt = canvas_.pixel(xi + 0, yi + 0).data();
        const data_type& rt = canvas_.pixel(xi + 1, yi + 0).data();
        const data_type& lb = canvas_.pixel(xi + 0, yi + 1).data();
        const data_type& rb = canvas_.pixel(xi + 1, yi + 1).data();

        for (unsigned int i = 0; i < pixel_format::channels(); ++i)
        {
            int t = (rt[i] - lt[i]) * xf + (lt[i] << shift());
            int b = (rb[i] - lb[i]) * xf + (lb[i] << shift());
