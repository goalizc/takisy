#ifndef grisu_h_20170227
#define grisu_h_20170227
...
class grisu {
    ...
    public:
        ...
        diyfp(double d) {
            union { double d; uint64_t u64; }

            u = { d };
            e = (u.u64 & DP_EXPONENT_MASK) >> DP_SIGNIFICAND_SIZE;
            f = (u.u64 & DP_SIGNIFICAND_MASK);
    ...
    public:
        ...
        diyfp& operator*=(const diyfp& rhs) {
                ...
                && defined(__x86_64__)
            f  = ((__uint128_t)f * (__uint128_t)rhs.f) >> 64;
        #else
    ...
    public:
        diyfp normalize(void) const {
            ...
            res.e  -= DIY_SIGNIFICAND_SIZE - DP_SIGNIFICAND_SIZE - 1;

            return res;
        }

        diyfp normalize_boundary(void) {
            diyfp res = *this;
        ...
        void normalized_boundaries(diyfp& minus, diyfp& plus) const {
            plus = diyfp((f << 1) + 1, e - 1).normalize_boundary();
    ...
    static int k_comp(int e, int alpha) {
        return ceil((alpha - e + 63) * D_1_LOG2_10);
    }

    static diyfp cached_power(int k) {
    ...
    static void digit_gen(const diyfp& w, const diyfp& p,
                          uint64_t delta, char* buffer, int& length, int& K) {
        static constexpr const uint64_t pow10[] = {
            1, 10, 100, 1000, 10000,
            100000, 1000000, 10000000, 100000000, 1000000000,
        };

        diyfp one(1ull << -p.e, p.e);
        uint32_t p1 = p.f >> -one.e;
        uint64_t p2 = p.f & (one.f - 1), pwf = (p - w).f, unit = 1;
        int d = 0, kappa = get_kappa(p1);
        ...
        while (kappa > 0) {
            ...
            if (d || length)
                buffer[length++] = '0' + d;
            uint64_t t = (((uint64_t)p1) << -one.e) + p2;
            if (t <= delta) {
                K += kappa;
                round(buffer, length, delta, t, pow10[kappa] << -one.e, pwf);
                return;
            }
        }

        while (true) {
            p2 *= 10; delta *= 10; unit *= 10; d = p2 >> -one.e;
            if (d || length)
                buffer[length++] = '0' + d;
            p2 &= one.f - 1; kappa--;
            if (p2 < delta) {
                K += kappa;
                round(buffer, length, delta, p2, one.f, pwf * unit);
                return;
            }
        }
    }

    static void grisu2(const diyfp& d, char* buffer, int& length, int& K) {
        diyfp m, p;
        d.normalized_boundaries(m, p);
        diyfp c_mk = cached_power(K = k_comp(p.e + 64, -59));

        p *= c_mk; p.f--;
        m *= c_mk; m.f++;

        digit_gen(d.normalize() *= c_mk, p, (p - m).f, buffer, length, K = -K);
    }
    ...
    static void prettify(char* buffer, int length, int K) {
        int k = length + K;
        ...
        if (length <= k && k <= 21) {
            ...
            buffer[1] = 'e';
            fill_exponent(k - 1, buffer + 2);
        } else {
            memmove(&buffer[2], &buffer[1], length - 1);
            buffer[1] = '.';
            buffer[length + 1] = 'e';
            fill_exponent(k - 1, buffer + length + 2);
        }
    }

public:
    static const char* strf(double d) {
        static char buffer[24]; char* ptr = buffer;
        int length = 0, K;
        ...
        grisu2(d, ptr, length, K);
        prettify(ptr, length, K);

        return buffer;
    }
};

#endif // grisu_h_20170227
